...
export default defineComponent({
	...
	data(vm) {
		vm.setActiveMenuHighlight = xU.debounce(function () {
			...
			const openKey = vm.pathAndIdCollection[currentPath];
			if (!openKey) {
				return;
			}
		...
		return { openKeys: [], pathAndIdCollection: {}, arr_selectedMenuId: [] };
	},
	methods: {
							...
							{{
								...
								default: () => {
									...
									if (currentMenuInfo.payload) {
										/*使用浏览器newTab展示新页面*/
										if (
											currentMenuInfo.payload.openType &&
											currentMenuInfo.payload.openType === STATIC_WORD.NEW_TAB
										) {
											const isDisabled = {};
	...
	render() {
		return <aMenu onOpenKeys={this.openKeys} onOpenChange={this.onOpenChange} v-model:selectedKeys={this.arr_selectedMenuId} mode="inline">
			{this.subTree}
		</aMenu>;
	}
});
